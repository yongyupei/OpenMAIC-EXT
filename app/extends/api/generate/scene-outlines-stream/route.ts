...
import { resolveGenerationMode } from '@/lib/slide-templates/resolve';
import { buildChapterStructureText, type ChapterHint } from '@/lib/teacher/preview-helpers';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return await aiTraceContext.run(
      {
        kind: 'preview-outline-stream',
        context: {
          userVisibleTitle: requirementSnippet,
        },
      },
      async () => {
    let effectivePdfText = pdfText ?? '';
    if (requirements.knowledgeNodeIds?.length) {
      const kbCtx = await resolveKnowledgeMountContext(requirements.knowledgeNodeIds);
      effectivePdfText = [kbCtx.referenceText, effectivePdfText].filter(Boolean).join('\n\n');
    }
    ...
    const generationMode = resolveGenerationMode(
      {},
      { generationMode: requirements.generationMode },
      effectivePdfText || undefined,
    );
    const materialDriven = generationMode === 'material-driven';
    const requirementDriven = generationMode === 'requirement-driven';
    const hybridMode = generationMode === 'hybrid';

    const prompts = buildPrompt(promptId, {
      requirement: requirements.requirement,
      pdfContent: effectivePdfText ? effectivePdfText.substring(0, MAX_PDF_CONTENT_CHARS) : 'None',
    ...
    // 教师模式：将章节结构注入 system prompt（追加，不修改模板）
