/**
 * @extends-from app/home/page.tsx
...
import { IdentityChoice, type IdentityMode } from '@/components/teacher/identity-choice';
import { storeTeacherHomepageRequirement } from '@/lib/teacher/homepage-handoff';
import { HomeRecentTabs } from '@/components/home/home-recent-tabs';
...
const IDENTITY_MODE_STORAGE_KEY = 'identityMode';
...
function HomePage() {
  ...
  const [identityMode, setIdentityMode] = useState<IdentityMode>('student');
  ...
  useEffect(() => {
    ...
    try {
      const savedMode = localStorage.getItem(IDENTITY_MODE_STORAGE_KEY);
      if (savedMode === 'teacher' || savedMode === 'student') {
  ...
  const updateIdentityMode = (mode: IdentityMode) => {
    setIdentityMode(mode);
    try {
      localStorage.setItem(IDENTITY_MODE_STORAGE_KEY, mode);
    } catch {
      /* ignore */
    }
  };
  ...
  const handleStudentEnter = async () => {
    try {
      ...
      const requirements: UserRequirements = {
        ...
        ...(form.knowledgeNodeIds.length > 0
          ? { knowledgeNodeIds: form.knowledgeNodeIds }
          : {}),
      };
  ...
  const handleTeacherEnter = () => {
    try {
      storeTeacherHomepageRequirement(form.requirement, form.knowledgeNodeIds);
    } catch (err) {
      log.error('Failed to persist teacher homepage requirement:', err);
    }
    router.push(buildTeacherNewPath());
  };
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    if ((e.metaKey || e.ctrlKey) && e.key === 'Enter') {
      e.preventDefault();
      if (canGenerate) handleEnterClassroom();
    }
  };
  ...
  return (
    <div className="min-h-[100dvh] w-full bg-gradient-to-b from-slate-50 to-slate-100 dark:from-slate-950 dark:to-slate-900 flex flex-col items-center p-4 pt-16 md:p-8 md:pt-16 overflow-x-hidden">
      ...
      >
        ...
        >
          <IdentityChoice mode={identityMode} onModeChange={updateIdentityMode} />
        </motion.div>
          ...
          <div className="w-full rounded-2xl border border-border/60 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl shadow-xl shadow-black/[0.03] dark:shadow-black/20 transition-shadow focus-within:shadow-2xl focus-within:shadow-violet-500/[0.06]">
            ...
            <div className="px-3 pb-3">
              <LlmComposerActionRow
                ...
                onPdfError={setError}
                knowledgeNodeIds={form.knowledgeNodeIds}
                onKnowledgeNodeIdsChange={(ids) => updateForm('knowledgeNodeIds', ids)}
                trailing={
                  <>
      ...
      <HomeRecentTabs
        ...
        importing={importing}
        defaultTab={identityMode === 'teacher' ? 'courses' : 'learning'}
      />
