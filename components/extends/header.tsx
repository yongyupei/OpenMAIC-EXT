/**
 * @extends-from components/header.tsx
...
import { ForkHomeEntryLink } from '@/components/home/fork-home-entry-link';
...
interface HeaderProps {
  ...
  /** When false, hides the left back / home control (e.g. teacher studio with its own nav). */
  readonly showBack?: boolean;
  /** When set, shows a labeled back link (e.g. teacher course workbench → home). */
  readonly backHref?: string;
  readonly backLabel?: string;
  /** When false, hides the light/dark/system theme toggle in the toolbar pill. */
  readonly showThemeToggle?: boolean;
  /** Extra controls inside the right toolbar pill (e.g. slide template picker). */
  readonly toolbarPillExtra?: ReactNode;
}
  ...
  return (
    <>
      <header className="h-20 px-8 flex items-center justify-between z-10 bg-transparent gap-4">
        <div className="flex items-center gap-3 min-w-0 flex-1">
          {showBack &&
            (backHref ? (
              <Link
                href={backHref}
                className="shrink-0 flex items-center gap-1.5 rounded-lg px-2 py-2 text-gray-500 transition-colors hover:bg-gray-100 hover:text-gray-800 dark:text-gray-400 dark:hover:bg-gray-800 dark:hover:text-gray-200"
              >
              ...
              <button
                type="button"
                onClick={() => router.push('/')}
                className="shrink-0 rounded-lg p-2 text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-700 dark:text-gray-500 dark:hover:bg-gray-800 dark:hover:text-gray-300"
        ...
        {toolbarExtra ? (
          <div className="flex min-w-0 max-w-[min(100%,56rem)] flex-nowrap items-center justify-end gap-2 overflow-x-auto overscroll-x-contain py-0.5">
            {toolbarExtra}
          </div>
        ) : null}
