/**
 * @extends-from components/canvas/canvas-area.tsx
...
interface CanvasAreaProps extends CanvasToolbarProps {
  ...
  /** Teacher Studio: preview HTML motion slides in iframe */
...
export function CanvasArea({
  ...
  const slideClickToPlay =
    showControls && !editable && currentScene?.type === 'slide' && !isLiveSession;
  const showPlayHint =
    slideClickToPlay && engineState !== 'playing' && !isPendingScene;

  const handleSlideClick = useCallback(
    (e: React.MouseEvent) => {
      if (!slideClickToPlay) return;
      ...
      >
        <div
          className={cn(
            ...
            slideClickToPlay && 'cursor-pointer',
            currentScene?.type === 'interactive'
            ...
            <div className="absolute inset-0">
              <SceneProvider>
                <SceneRenderer
                  ...
                  mode={mode}
                  editable={editable}
                  htmlSlidePreview={htmlSlidePreview}
                />
              </SceneProvider>
            </div>
          )}
