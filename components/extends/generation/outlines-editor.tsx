/**
 * @extends-from components/generation/outlines-editor.tsx
...
interface OutlinesEditorProps {
  ...
  onCollapse?: () => void;
  /** Fill parent height: scene list scrolls internally; page stays fixed (chapter outline review). */
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        embeddedLayout
          ? 'grid h-full max-h-full min-h-0 grid-rows-[minmax(0,1fr)_auto] overflow-hidden'
          : undefined,
      )}
    >
      {/* Soft gradient wash */}
      ...
      {/* Header — hidden in embedded chapter review (page already shows title). */}
      {!embeddedLayout ? (
      <div className="relative flex shrink-0 items-start gap-3 px-6 pt-6 pb-4 md:px-10 md:pt-8 md:pb-6">
      ...
      {/* Footer */}
      <div className="relative z-10 flex shrink-0 flex-col gap-3 border-t border-border/40 bg-background/95 px-6 py-4 backdrop-blur-sm md:flex-row md:items-center md:justify-between md:px-10 md:py-5">
        <label
          className={cn(
