/**
 * @extends-from components/chat/chat-area.tsx
 * @fork-branch feat/html-slide-design-workbench
 */
'use client';

import { useImperativeHandle, forwardRef, useRef, useCallback, useState, useMemo, type CSSProperties } from 'react';
...
interface ChatAreaProps {
  ...
  /** Chapter studio 1:3:1 layout — flex column instead of fixed pixel width. */
  proportionalLayout?: boolean;
  /** Studio editor: edit speech in the notes panel. */
  lectureNotesEditable?: boolean;
}
  ...
  (
    ...
    const lectureNotes: LectureNoteEntry[] = useMemo(
      () =>
        scenes
          .filter(
            (scene) =>
              lectureNotesEditable || (scene.actions && scene.actions.length > 0),
          )
          .map((scene) => ({
            sceneId: scene.id,
    ...
    return (
      <div
        style={panelStyle}
        className={cn(
          'bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl border-l border-gray-100 dark:border-gray-800 shadow-[-2px_0_24px_rgba(0,0,0,0.02)] flex flex-col z-20 relative overflow-visible',
