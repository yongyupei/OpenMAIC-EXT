/**
 * @extends-from lib/utils/stage-storage.ts
...
import { Stage, Scene } from '@/lib/types/stage';
import { ChatSession } from '@/lib/types/chat';
import { db } from '@/lib/utils/database';
import { saveChatSessions, loadChatSessions, deleteChatSessions } from '@/lib/utils/chat-storage';
import { clearPlaybackState } from '@/lib/utils/playback-storage';
...
type ThumbnailSlide = import('@/lib/types/slides').Slide;
    ...
    await Promise.all(
      stageIds.map(async (stageId) => {
        ...
        if (firstSlide && firstSlide.content.type === 'slide') {
          ...
          result[stageId] = slide as ThumbnailSlide;
        }
      }),
    );
  } catch (error) {
