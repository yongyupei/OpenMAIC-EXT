/**
 * @extends-from lib/types/stage.ts
...
/**
 * Full-page HTML slide for teacher Studio preview (student classroom ignores this field).
 */
export interface HtmlSlideDocument {
  html: string;
  themeId?: string;
...
export interface SlideContent {
  ...
  canvas: Slide;
  /** Teacher Studio HTML motion slide; not rendered in student classroom (Phase 1). */
  htmlSlide?: HtmlSlideDocument;
  /** Pipeline marker: canvas vs html generation */
  slideOutputFormat?: import('@/lib/teacher/slide-output-format').SlideOutputFormat;
  /**
   * Immutable copy of `canvas` as first built by generation (`buildCompleteScene` / `createSceneWithActions`).
   * Used by restore-to-default to replay the original slide, not re-derive colors from theme rules.
   */
  generationSnapshot?: Slide;
  /** Slide template id that was active when `generationSnapshot` was captured. */
  generationSlideTemplateId?: string;
}
