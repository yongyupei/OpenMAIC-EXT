/**
 * @extends-from lib/types/slides.ts
 ...
 *
 * fontColor: 正文字体颜色（与 bodyFontColor 等价，保留兼容）
 *
 * fontName: 主字体（与 bodyFontName 等价，保留兼容）
 *
 * titleFontColor / bodyFontColor: 标题与正文颜色，替换模板时按 textType 应用
 *
 * titleFontName / bodyFontName: 标题与正文字体
 *
 * accentFontColor: 脚注、页码等次要文字颜色
 *
 * contentBlockColors: 内容色块 / 形状填充色板（按槽位 0→n 对应）
 *
 * mutedBlockFill: 浅色衬底或大面板背景块
 *
 * lineColor / blockOutlineColor: 分隔线与色块描边
 *
 * chartColors: 图表系列色（默认同 themeColors）
 */
export interface SlideTheme {
  ...
  bodyFontName?: string;
  contentBlockColors?: string[];
  /** Saturated accent hues paired with contentBlockColors slots (card rails / charts). */
  blockAccentHues?: string[];
  mutedBlockFill?: string;
  lineColor?: string;
  blockOutlineColor?: string;
  chartColors?: string[];
  outline?: PPTElementOutline;
  shadow?: PPTElementShadow;
}
