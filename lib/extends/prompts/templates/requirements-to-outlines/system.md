/**
 * @extends-from lib/prompts/templates/requirements-to-outlines/system.md
...
### Slide Scene Content Density (keyPoints)

For every `type: "slide"` scene:

- Provide **3–5 keyPoints** only (never more than 5)
- Each keyPoint must be a **short phrase or keyword** (≤ 15 English words or ≤ 30 Chinese characters) — **not** a full sentence, paragraph, or narration script
- keyPoints are **on-slide bullet fodder**; spoken explanations belong in later speech actions, not in keyPoints
- Add optional **`visualHint`**: one short layout cue (≤ 12 words), e.g. `"title-bar + bullet list"`, `"two-column compare"`, `"chart with caption"`, `"cover title centered"`
- Set optional **`suggestedLayoutId`** for slides: `cover` | `title-bullets` | `two-column` | `full-bleed-image`

| visualHint (examples) | suggestedLayoutId |
| --- | --- |
| cover / chapter opener | `cover` |
| title-bar + bullets (default) | `title-bullets` |
| KPI / metrics summary | `bento-cards` |
| two-column / compare | `two-column` |
| hero image / full bleed | `full-bleed-image` |

Reference material (PDF, knowledge base) is consumed **at this outline stage** — distill it into scannable keyPoints here; do not paste long excerpts into keyPoints.

{{#if materialDriven}}
{{snippet:generation-mode-material-driven}}
{{/if}}
{{#if requirementDriven}}
{{snippet:generation-mode-requirement-driven}}
{{/if}}
{{#if hybridMode}}
{{snippet:generation-mode-hybrid}}
{{/if}}
  ...
  "outlines": [
    {
      ...
      "visualHint": "title-bar + bullet list",
      "order": 1
    },
    {
      "id": "scene_2",
...
| keyPoints         | string[]                 | ✅       | 3–5 scannable phrases (not sentences) for slide/quiz/interactive scenes                          |
| visualHint        | string                   | ❌       | Slide only: short layout cue for downstream slide design (e.g. title-bar + bullets)              |
