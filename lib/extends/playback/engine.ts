/**
 * @extends-from lib/playback/engine.ts
 * @fork-branch feat/html-slide-design-workbench
 *
 * Course-editor fork: browser TTS fallback must not be gated by ttsEnabled
 * (that flag only controls generation-time synthesis, not playback).
 */
/**
...
import { generateSpeechAudioOnDemand } from '@/lib/extends/playback/on-demand-tts';
    ...
    switch (action.type) {
      case 'speech': {
        ...
        const playSpeech = async () => {
          try {
            if (
              await this.audioPlayer.play(speechAction.audioId || '', speechAction.audioUrl)
            ) {
              return;
            }
          } catch (err) {
            log.warn('Pre-generated speech audio failed:', err);
          }

          if (this.mode !== 'playing') return;

          try {
            const onDemandId = await generateSpeechAudioOnDemand(speechAction);
            if (this.mode !== 'playing') return;
            if (onDemandId && (await this.audioPlayer.play(onDemandId))) {
              return;
            }
          } catch (err) {
            log.warn('On-demand TTS failed:', err);
          }
