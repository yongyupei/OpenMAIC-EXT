/**
 * @extends-from lib/store/stage.ts
...
const useStageStoreBase = create<StageState>()((set, get) => ({
  ...
  loadFromStorage: async (stageId: string) => {
    try {
      ...
      if (data) {
        const { ensureGenerationSnapshotsOnLoad } = await import(
          '@/lib/generation/slide-generation-snapshot'
        );
        set({
          stage: data.stage,
          scenes: ensureGenerationSnapshotsOnLoad(data.scenes),
