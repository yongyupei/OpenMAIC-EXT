/**
 * @extends-from lib/server/classroom-generation.ts
...
import {
  ...
  resolveSlideTemplateById,
} from '@/lib/slide-templates/resolve';
...
export async function generateClassroom(
  ...
  const requirements: UserRequirements = {
    requirement,
    ...(input.knowledgeNodeIds?.length ? { knowledgeNodeIds: input.knowledgeNodeIds } : {}),
    ...(input.slideTemplateId ? { slideTemplateId: input.slideTemplateId } : {}),
  ...
  const pdfText =
    [kbText, pdfContent?.text].filter(Boolean).join('\n\n') || undefined;
  ...
  const resolvedTemplate = await resolveSlideTemplateById(
    requirements.slideTemplateId ?? input.slideTemplateId,
  );
